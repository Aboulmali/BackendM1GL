...
{
    // ═══════════════════════════════════════════════════════════════
    // ── Profil personnel (utilisateur connecté) ──────────────────
    // ═══════════════════════════════════════════════════════════════
    ...
    public async Task<ActionResult<UserDto>> UpdateMe(UpdateProfileDto dto)
    {
        ...
        log.LogInformation("Profil mis à jour : {Email}", user.Email);
    ...
    public async Task<IActionResult> ChangePassword(ChangePasswordDto dto)
    {
        ...
        {
            log.LogWarning("Tentative changement mot de passe échouée : {Email}", user.Email);
        ...
        log.LogInformation("Mot de passe changé : {Email}", user.Email);
        return NoContent();
    }
    ...
    public async Task<ActionResult<UserDto>> Update(Guid id, UpdateUserDto dto)
    {
        ...
        await db.SaveChangesAsync();
        log.LogInformation("Utilisateur modifié par admin : {Email}", user.Email);
    ...
    [Authorize(Roles = "Admin")]
    public async Task<ActionResult<UserDto>> SetRole(Guid id, SetRoleDto dto)
    {
        var user = await db.Users.FindAsync(id);
        if (user is null) return NotFound();
        ...
        log.LogInformation("Rôle changé : {Email} → {Role}", user.Email, dto.Role);
        return Ok(ToDto(user));
    }
    ...
    public async Task<IActionResult> ToggleActive(Guid id)
    {
        ...
        log.LogInformation("Statut user changé : {Email} → IsActive={Active}",
    ...
    public async Task<IActionResult> Delete(Guid id)
    {
        ...
        log.LogWarning("Utilisateur supprimé : {Email}", user.Email);
        return NoContent();
    }
    ...
    public async Task<IActionResult> Stats()
    {
        ...
        var today = await db.Users.CountAsync(u =>
    ...
    private static UserDto ToDto(User u) =>
        new(u.Id, u.FirstName, u.LastName, u.Email, u.Role, u.IsActive,
            u.AvatarUrl, u.Phone, u.Department, u.CreatedAt);
}